...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (issilicon(usr)))
		...
		if(href_list["set_channel_name"])
			...
			src.msg = sanitize(input(usr, "Write your Feed story", "Network Channel Handler", "", "message"))
			src.updateUsrDialog()
			...
			src.msg = html_encode(input(usr, "Provide the a description of the Wanted person and any other details you deem important", "Network Security Handler", "", "message"))
			src.updateUsrDialog()
